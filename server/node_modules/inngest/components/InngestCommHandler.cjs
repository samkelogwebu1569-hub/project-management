...
var InngestCommHandler = class {
	...
	*/
	async initRequest(...args) {
		...
		this.env = require_env.protectEnv({
			...require_env.getProcessEnv(),
			...env
		});
		this.client.setEnvVars(this.env);
...
var RequestSignature = class {
	...
	async #verifySignature({ body, signingKey, allowExpiredSignatures, logger }) {
		...
		if (!require_strings.timingSafeEqual(await require_net.signDataWithKey(body, signingKey, this.timestamp, logger), this.signature)) throw new Error("Invalid signature");
	}
